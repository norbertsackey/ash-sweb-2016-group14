...
/**
   ...
   */ 
	...
	switch($cmd){
		case 1:
			deleteLab();	//if cmd=1 the call delete
			break;
		case 2:
			labStatus();	//if cmd=2 the change status
		default:
	...
	function deleteLab(){
		...
		if(!isset($_REQUEST['Lab_Id'])){
			echo "lab_id is not given";	//change to json message	
			exit();
		}
		
		$userid=$_REQUEST['Lab_Id'];
		include_once("labs.php");
		$obj = new labs();
		//delete the user
		if($obj->deleteLab($lad_id)){
			echo "Lab has been successfully deleted";
		}
		else{
			echo "Lab deletion unsuccessful ";
		}
	}
	...
	function LabStatus(){
		...
		if(!isset($_REQUEST['status'])){
			echo '{"message":"lab code is not correct"}';
			return;
		}
		...
		//return json message
		echo '{"result":1,"message":"lab status changed"}';
