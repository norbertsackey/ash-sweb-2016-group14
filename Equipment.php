...
class Equipment extends adb {
    ...
    function searchEquipment($searchTerm) {
        $strQuery = "select EquipID,name,descrip,price,category,manufacturer,availQuantity,totalQuantity,status,supplierID from equipment where name like '%$searchTerm%' or descrip like '%$searchTerm%' or category like '%$searchTerm%';";
        return $this->query($strQuery);
    }
    ...
    function bookEquipment($EquipID) {
        $newAvailQuantity = $this->getAvailQuantity($EquipID);
        $newAvailQuantity++;
